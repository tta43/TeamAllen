...
void CWall::Render(void)
{
	...
	m_pBuffer[WALL_LEFT]->Render();

	// ¿À¸¥ÂÊ
	m_pInfo->m_matWorld._41 = 40.f;
	m_pDevice->SetTransform(D3DTS_WORLD, &m_pInfo->m_matWorld);
	m_pTexture->Render(0, 0);
	m_pBuffer[WALL_RIGHT]->Render();

	// ¾Õ
	m_pInfo->m_matWorld._41 = -10.f;
	m_pInfo->m_matWorld._42 = -10.f;
	m_pInfo->m_matWorld._43 = 700.f;
	m_pDevice->SetTransform(D3DTS_WORLD, &m_pInfo->m_matWorld);
	m_pTexture->Render(0, 0);
