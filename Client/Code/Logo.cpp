...
HRESULT CLogo::Initialize(void)
{
	...
	// 스레드에서 하면 좋을듯(디바이스 설정 바꾸는것 잊지 말고)
	hr = Add_Buffer();
	hr = Add_Texture();
...
HRESULT CLogo::Add_Buffer(void)
{
	...
	hr = Engine::Get_ResourceMgr()->AddBuffer(m_pDevice, Engine::RESOURCE_STATIC,
		Engine::BUFFER_WALLTEX, L"Buffer_LeftWallTex", VTXCNTX, VTXCNTY, VTXCNTZ, VTXITV, D3DXVECTOR3(1.f, 0.f, 0.f));
	FAILED_CHECK(hr);

	hr = Engine::Get_ResourceMgr()->AddBuffer(m_pDevice, Engine::RESOURCE_STATIC,
		Engine::BUFFER_WALLTEX, L"Buffer_RightWallTex", VTXCNTX, VTXCNTY, VTXCNTZ, VTXITV, D3DXVECTOR3(-1.f, 0.f, 0.f));
	FAILED_CHECK(hr);
...
HRESULT CLogo::Add_Texture(void)
{
	...
	hr = Engine::Get_ResourceMgr()->AddTexture(m_pDevice, Engine::RESOURCE_STATIC, Engine::TEXTURE_CUBE
		, L"Texture_Cube", L"../bin/Texture/Tile/Tile%d.dds", 23);
	FAILED_CHECK(hr);
